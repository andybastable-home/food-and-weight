...
async function ensureSheet() {
  ...
  await apiCall(
    ...
    { method: 'POST', body: JSON.stringify({ values: [ENTRIES_HEADER_V2] }) }
  );
  await apiCall(
  ...
  await apiCall(
    `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/Metadata!A1?valueInputOption=RAW`,
...
async function pullEntriesFromSheet() {
  ...
  try {
    ...
    if (version == null) {
      // No Metadata tab → either legacy v1 or a foreign sheet. Header check + migrate.
      ...
      // Should be impossible (lowest is v2) — but if it ever happens, treat like legacy.
      await migrateSheetV1ToV2();
    }
    schemaCompatible = true;
...
async function attachToSheet(input) {
  ...
  // Validate schema. Missing Metadata → must be v1 with the exact legacy header, or refuse.
  const version = await readSheetSchemaVersion(sheetId);
  if (version == null) {
    const headerRead = await apiCall(
      `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Entries!A1:I1`
    ...
    const headerRow = headerRead.values?.[0] || [];
    if (!headersMatch(headerRow, ENTRIES_HEADER_V1) && !headersMatch(headerRow, ENTRIES_HEADER_V2)) {
      clearSheetId();
      clearSheetGid();
